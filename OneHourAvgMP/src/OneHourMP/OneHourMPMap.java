...
public class OneHourMPMap extends Mapper<LongWritable, Text, Text, Text> {
	...
	public void map(LongWritable key, Text value, Context context)   throws IOException, InterruptedException {
		...
		try {
			...
			//the 3rd argument set in main is used to identify the pollutants
			Configuration conf = context.getConfiguration();
			String param_arg = conf.get("param_id");
			String [] pollutants = new String[20];
			pollutants = param_arg.toString().split(",");
			List poll_list = Arrays.asList(pollutants);
			...
			if(HoustonSitesList.contains(siteStr))
			{
				...
				if(poll_list.contains(param_idStr))//write output key, value
